...
lutar pela Guerrilha pelo Acesso Aberto.`}
        </p>
        <p>
          {`Se somarmos muitos de nós, não vamos apenas enviar uma forte mensagem de oposição
à privatização do conhecimento – vamos transformar essa privatização em algo do
passado. Você vai se juntar a nós?`}
        </p>
        <p>{`Aaron Swartz`}</p>
        <p>{`Júlio de 2008, Eremo, Itália`}</p>
      </section>
    </Layout>
  );
}
